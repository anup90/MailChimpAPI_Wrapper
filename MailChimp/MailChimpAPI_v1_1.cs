...
namespace MailChimp
{
			...
			try
			{
				...
				foreach (DummyMCTemplate t in tmp)
				{
					...
					else if (t.sections is XmlRpcStruct)
					{
						...
						toret[i].sections = new string[(s).Count];
	...
	public interface IMC : IXmlRpcProxy
	{
		...
		bool listSubscribe(string apikey, string id, string email_address, XmlRpcStruct mv, string email_type, bool double_optin);
		...
		DummyMCBatchResult listBatchSubscribe(string apikey, string id, XmlRpcStruct batch, bool double_optin, bool update_existing, bool replace_interests);

		[XmlRpcMethod("listBatchUnsubscribe")]
		DummyMCBatchResult listBatchUnsubscribe(string apikey, string id, XmlRpcStruct batch, bool delete_member, bool send_goodbye, bool send_notify);

		[XmlRpcMethod("listUpdateMember")]
		bool listUpdateMember(string apikey, string id, string email, XmlRpcStruct mv, string email_type, bool replace_interests);
